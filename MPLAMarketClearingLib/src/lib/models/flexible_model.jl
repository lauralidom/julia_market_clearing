...
# m, time_period, resultset, initialization, data, market
function define_sets!(m::Model, time_period::Int, resultset, initialization::Dict{Symbol,Any}, data::Dict{Symbol,Any}, market::Dict{Symbol,Any})
...
# TODO: for each of the inputs, also store the current dispatch so that the quantities can be adjusted if necessary - from resultset

# Step 2b: process time-series data (turn YAML parameters into hourly prices/quantities)
function process_time_series_data!(m::Model, time_period::Int, resultset, initialization::Dict{Symbol,Any}, data::Dict{Symbol,Any}, market::Dict{Symbol,Any})
    disp_gen = data[:dispatchableGenerators]
    var      = data[:variableGenerators]
    dem      = data[:demandSegments]
    
    CH = m.ext[:sets][:CH] # CH is the set of periods for the clearing window
    ...
    for (dname, ddata_any) in dem
        ...
        for t in CH
            ...
            Q_prev_dem[(d,t)] = ProcessData.DemPreviousDispatchDataForTimePeriod(resultset, d, t)
        end
    end
...
function process_parameters!(m::Model, time_period::Int, resultset, initialization::Dict{Symbol,Any}, data::Dict{Symbol,Any}, market::Dict{Symbol,Any})
    ...
    if data[:batteryStorage] !== nothing
        ...
        previous_SOC_outcomes = ProcessData.StorageStateOfChargeOutcomes(resultset)
        start_SOC = time_period < 1 + length(previous_SOC_outcomes) ? previous_SOC_outcomes[time_period - 1] : initialization[:SOC]
    ...
    for (g, gen_config) in data[:dispatchableGenerators]
        ...
        m.ext[:parameters][:previous_time_period_dispatch][g] = initialization[:Q_gen][g]
    end
...
function build_market_clearing!(m::Model, time_period::Int, resultset, initialization::Dict{Symbol,Any}, data::Dict{Symbol,Any}, market::Dict{Symbol,Any})
    ...
    start_at_period = m.ext[:sets][:CH][1]
    # Storage constraints
    if has_storage
        η = m.ext[:parameters][:storage_efficiency]
...
function build(time_period, resultset, initialization, data, market)
	...
	define_sets!(m, time_period, resultset, initialization, data, market)
	process_time_series_data!(m, time_period, resultset, initialization, data, market)
	process_parameters!(m, time_period, resultset, initialization, data, market)

	# create variables, constraints and objective, then solve
	build_market_clearing!(m, time_period, resultset, initialization, data, market)
