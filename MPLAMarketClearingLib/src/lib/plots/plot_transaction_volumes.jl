...
function plot(resultset, clearing_time_period, market_name)
    ...
    for transaction in transactions
    	...
    	if transaction.ClearingTimePeriod == clearing_time_period && transaction.PartyType != HelperModelResults.PARTY_DEMAND
    		stack_matrix[2, transaction.TimePeriod] += transaction.Quantity # quantity dispatched in this clearing period for each period considered, half of abs so they don't cancel each other out, subtracted so that it is shown as a portion of the total dispatched generation
    	# this is all other generation transactions that have happened for this period 
    	elseif transaction.PartyType != HelperModelResults.PARTY_DEMAND
    ...
    p3 = Plots.plot(xlabel="Time Period", ylabel="Volume Dispatched (MWh)",
            title="Quantities Dispatched in " * market_name,
            legend=:topright)
